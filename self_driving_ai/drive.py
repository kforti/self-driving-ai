...
class SimplePIController:
    ...
    def update(self, measurement):
        ...
        # Custom logic for handling when the vehicle stalls
        if measurement < 0.5:
            value = -value
        return value
...
def telemetry(sid, data):
    if data:
        ...
        image = transform_pipe(image)
        # predict the steering angle and throttle for the image
        data = model(image.view(1, 3, 224, 224))
